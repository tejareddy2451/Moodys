...
def bulk_upsert(upsert_tables_list, read_df_dict):
    ...
    for upsert_table_info in upsert_tables_list:
        ...
        postgres_util.upsert(write_df, postgres_options, target_table_name, upsert_table_info['unique_key_list'],
...
def main():
    ...
    meth_query = f"""select nextval('meth_id_seq'), a.*
...
where Methodology_Document_Version_Number = (select max(Methodology_Document_Version_Number)
    ...
    meth_upsert_table = [{
        'target_table': 'meth',
        'unique_key_list': ['meth_id', "meth_nm"],
    ...
    # meth_doc
    meth_doc_query = f"""select nextval('meth_doc_id_seq'),
	   meth_id,
...
where trim(a.Methodology_Name) = b.meth_nm
  ...
  order by meth_id,Methodology_Document_Publication_Datetime;"""
    ...
    meth_doc_upsert_table = [{
        ...
        'unique_key_list': ['meth_id', 'lang_iso_alpha_3_cd', 'publ_entity_cd', 'meth_vrsn_num', 'meth_doc_typ_cd'],
    ...
    # rfc_doc
    rfc_doc_query = f"""select nextval('rfc_id_seq'),
...
where Request_for_Comment_CMS_Document_Identifier = rfc_cms_doc_id	 
  ...
  and a.Result_of_Consultation_Status_Code = c.roc_status_txt;"""
    ...
    roc_doc_upsert_table = [{
        ...
        'unique_key_list': ['rfc_id', 'roc_cms_doc_id'],
